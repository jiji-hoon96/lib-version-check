...
async function fetchPackageInfo(packageName) {
	try {
		...
		console.error(
			chalk.red(`Error fetching info for ${packageName}: ${error.message}`),
		);
		return null;
	}
}
...
program
	...
	.action(async (options) => {
		if (options.show) {
			...
			console.log(
				chalk.green(
					`User ID changed from "${oldUserId}" to "${answer.newUserId}"`,
				),
			);
	...
	.action(async () => {
		...
		try {
			...
			if (!userLibraries.includes(answer.library)) {
				...
				console.log(
					chalk.yellow(`${answer.library} is already in your watch list`),
				);
			}
		} catch (error) {
	...
	.action(async () => {
		...
		async function explorePresets() {
			...
			if (categoryAnswer.category === "contribute") {
				...
				console.log(
					chalk.yellow(
						"   https://github.com/jiji-hoon96/lib-version-check/pulls\n",
					),
				);
				...
				console.log(
					chalk.yellow(
						"https://github.com/jiji-hoon96/lib-version-check/compare\n",
					),
				);
			...
			const presetAnswer = await inquirer.prompt([
				{
					...
					choices: [
						...Object.entries(PRESETS[categoryAnswer.category]).map(
							([key, value]) => ({
								name: value.name,
								value: key,
							}),
			...
			const selectedPreset =
				PRESETS[categoryAnswer.category][presetAnswer.preset];
			...
			for (const pkg of selectedPreset.packages) {
				try {
					...
					console.error(
						chalk.red(`Error fetching info for ${pkg}: ${error.message}`),
					);
				}
			}
			...
			packageDetails.forEach((pkg) => {
				console.log(
					chalk.green(
						`${pkg.name}${
							pkg.inWatchList ? chalk.yellow(" (Already in watch list)") : ""
						}:`,
					),
				);
...
program
	...
	.action(async () => {
		...
		if (index > -1) {
			...
			console.log(
				chalk.green(`Removed ${answer.library} from your watch list`),
			);
		}
	});
...
program
	...
	.action(async (options) => {
		...
		if (userLibraries.length === 0) {
			console.log(
				chalk.yellow(
					"No libraries in your watch list. Add one with: lib-check add",
				),
			);
			return;
		}
			...
			if (info) {
				...
				if (options.detailed) {
					...
					if (deps) {
						if (deps.bundleSize) {
							console.log(
								`  Bundle size: ${(deps.bundleSize.size / 1024).toFixed(
									1,
								)}KB (${(deps.bundleSize.gzip / 1024).toFixed(1)}KB gzipped)`,
							);
						}
						if (deps.vulnerabilities && deps.vulnerabilities.length > 0) {
							console.log(
								chalk.red(
									`  Vulnerabilities found: ${deps.vulnerabilities.length}`,
								),
							);
						}
						console.log(
							"  Dependencies:",
							Object.keys(deps.dependencies).length,
						);
						console.log(
							"  Peer Dependencies:",
							Object.keys(deps.peerDependencies).length,
						);
					}
				}
...
program
	.command("preview")
	.description(
		"Preview information about packages in a preset without adding them",
	)
	.option("-d, --detailed", "Show detailed dependency information")
	.action(async (options) => {
		try {
			...
			console.log(
				chalk.yellow(
					"Calculating total bundle size and analyzing dependencies...\n",
				),
			);
			...
			if (options.detailed) {
				...
				console.log(
					`Total unique peer dependencies: ${totalPeerDependencies.size}`,
				);
				console.log(
